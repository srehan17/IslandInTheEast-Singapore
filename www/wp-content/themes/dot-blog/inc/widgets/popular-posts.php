...
/**
 * Widget : Dot_Blog_Popular_Posts
 *
 */
class Dot_Blog_Popular_Posts extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
			'classname'                   => 'recent-posts-widget',
			'description'                 => __( 'Your site&#8217;s popular Posts.', 'dot-blog' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'dot-blog-popular-posts', __( 'Dot - Popular Posts', 'dot-blog' ), $widget_ops );
		$this->alt_option_name = 'dot_blog_popular_posts';
	}
	...
	public function widget( $args, $instance ) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Popular Posts', 'dot-blog' );
	...
	public function form( $instance ) {
		...
		<p><input class="checkbox" type="checkbox"<?php checked( $show_date ); ?>
			...
			<label
				for="<?php echo $this->get_field_id( 'show_date' ); ?>"><?php esc_html_e( 'Display post date?', 'dot-blog' ); ?></label>
		</p>

		<p><input class="checkbox" type="checkbox"<?php checked( $show_cats ); ?>
		          id="<?php echo $this->get_field_id( 'show_cats' ); ?>"
		          name="<?php echo $this->get_field_name( 'show_cats' ); ?>"/>
			<label
				for="<?php echo $this->get_field_id( 'show_cats' ); ?>"><?php esc_html_e( 'Display post cat?', 'dot-blog' ); ?></label>
		</p>
		<?php
	}
}
