...
class LMM_WP_Session_Utils {
	...
	public static function delete_old_sessions( $limit = 2000 ) {
		...
		if ($keys != NULL) { //info: as PHP warnings are thrown otherwise
		...
		// Delete expired sessions
 		if ( ! empty( $expired ) ) {
			$wpdb->query(
				"DELETE FROM $wpdb->options WHERE option_name IN ('".implode( "','", $expired )."')"
			);
 		}
